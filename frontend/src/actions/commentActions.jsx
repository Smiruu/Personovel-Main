...
export const getRepliesForComment = (commentId) => async (dispatch) => {
  console.log("GET_REPLIES_REQUEST dispatched"); // Log dispatch
  dispatch({ type: GET_REPLIES_REQUEST, commentId });
  try {
    const { data } = await instance.get(`/api/replies/comment/${commentId}/`);
    console.log("Replies payload:", data); // Log the payload
    dispatch({ type: GET_REPLIES_SUCCESS, payload: data, commentId });
  } catch (error) {
    dispatch({ type: GET_REPLIES_FAIL, payload: error.message, commentId });
  }
};
